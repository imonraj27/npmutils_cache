const { TimeUnit, convertTime } = require("../helpers/convert_helper");

class InMemoryCache {
    // PRIVATE VARIABLES 
    #cache; #defaultTtl_minutes;
    ...
    constructor(defaultTtl = 10, ttlTimeUnit = TimeUnit.MINUTE) {
        if (InMemoryCache.instance) {
            InMemoryCache.instance.#defaultTtl_minutes = convertTime(defaultTtl, ttlTimeUnit, TimeUnit.MINUTE);
            return InMemoryCache.instance;
        }

        this.#cache = new Map();
        this.#defaultTtl_minutes = convertTime(defaultTtl, ttlTimeUnit, TimeUnit.MINUTE);
     ...
     */
    set(key, value) {
        const expirationTime = Date.now() + this.#defaultTtl_minutes * TimeUnit.MINUTE.getMs();
